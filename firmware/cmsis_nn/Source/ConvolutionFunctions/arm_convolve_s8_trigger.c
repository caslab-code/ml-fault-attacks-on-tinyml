...
arm_cmsis_nn_status arm_convolve_s8_trigger(const uint8_t layer_fault_no,
                                            ...
                                            int8_t *output_data)
{
    ...
    uint16_t conv_gemm_no = 0;   // reset every time this conv layer runs
    ...
    for (int i_batch = 0; i_batch < input_batches; i_batch++)
    {
        ...
        for (int32_t i_group = 0; i_group < groups; i_group++) // loop run once
        { // 422
            int8_t *out = output_data + i_group * output_ch_per_group; 
            for (int i_out_y = 0; i_out_y < output_y; i_out_y++) // loop run 6 times -> we fault in the 2nd iteration of this loop -> 24,000 cycles
            { // 441
                for (int i_out_x = 0; i_out_x < output_x; i_out_x++) // loop run 6 times
                { // 465
                    const int32_t base_idx_x = stride_x * i_out_x - pad_x;
                    const int32_t base_idx_y = stride_y * i_out_y - pad_y;

                    for (int32_t i_ker_y = 0; i_ker_y < kernel_y; i_ker_y++) // loop run 3 times
                    { // 487
                        for (int32_t i_ker_x = 0; i_ker_x < kernel_x; i_ker_x++) // loop run 3 times
                        {
    ...
    // #endif
                    ...
                    {
                        conv_gemm_no++;
                        int do_fault = (layer_fault_no == conv_gemm_no);
